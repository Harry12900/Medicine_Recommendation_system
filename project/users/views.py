...
def getMedicines(dic, prediction):
    df = pd.read_csv('model\medications.csv')
...
def suggestedDiets(dic, prediction):
    df = pd.read_csv('model\diets.csv')
    for key, value in dic.items():
        if value == prediction[0]:
            for i in range(len(df)):
                row = df.iloc[i, :2]
...
def create_map(latitude, longitude, hospitals):
    ...
    for hospital in hospitals:
        ...
        <div style="font-size: 16px; line-height: 1.5; color: #333; background-color: #fff; border-radius: 8px; padding: 10px; border: 1px solid #ddd;">
            ...
            <a href='https://www.google.com/maps/dir/?api=1&origin={latitude},{longitude}&destination={hosp_lat},{hosp_lon}' target='_blank' style="color: blue; text-decoration: underline;">Get Directions</a>
        </div>
        """
        folium.Marker(
            [hosp_lat, hosp_lon],
...
def data():
    # Corrected path
    path = os.path.join("project","static", "hospitals.html")
